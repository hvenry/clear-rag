    ...
    <div className="glass-strong border-x-0 border-t-0 px-5 py-3">
      {failures.map((check) => (
        <div key={check.component} className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
          <span className="flex items-center gap-1.5 font-display text-[10px] tracking-[0.18em] uppercase">
            <WarningIcon size={12} weight="fill" className="text-critical" aria-hidden />
            {check.component}
          </span>
