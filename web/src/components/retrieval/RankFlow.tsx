...
export function RankFlow({ stages, onSelect, selected }: RankFlowProps) {
  ...
  // Only as tall as the ranks actually drawn: a five-candidate run does not
  // need twelve rank rows of empty grid below its lines.
  const rankRows = Math.min(
    MAX_RANK,
    Math.max(1, ...tracks.flatMap((t) => t.points.map((p) => p.rank)))
  );
  const height = PAD.top + rankRows * ROW + PAD.bottom;
