    ...
    topics: [
      ...
      },
      {
        ...
        body: [
          ...
          "**A second embedder**: `ollama pull mxbai-embed-large` (1024-dim vs nomic's 768) or `all-minilm` (384, the predecessor's family) and `clear-rag ablate` adds a dense-only and a hybrid + RRF row under each, on an index of its own — the embedder is the identity of an index, not a knob, so it rides on the row's provenance rather than its config hash. Pin nomic's row in Results and the arrows say what the model was worth, question by question. Embedding choice is the single most cargo-culted decision in RAG; here it is a table row.",
