...
export const TOPIC_GROUPS: TopicGroup[] = [
  {
    ...
    topics: [
      ...
      },
      {
        ...
        summary: "Two searches, one ranking, merged by rank rather than by score.",
        body: [
          "Keyword and vector search fail differently: one misses synonyms, the other misses exact identifiers. Hybrid retrieval runs both and merges the rankings. On the bundled benchmark, that merge is the whole story: recall@5 goes from 0.871 (vector alone) and 0.903 (keyword alone) to 0.968 combined, and every distractor question that either method alone got wrong is resolved.",
          "The merge cannot simply add scores: BM25 scores are unbounded sums while cosine lives in [−1, 1], so any weighting of raw scores needs per-corpus tuning that rots as the corpus changes. Reciprocal Rank Fusion sidesteps this by consuming only rank positions. A chunk both searches liked beats a chunk only one search ranked first, because agreement between independent methods is the strongest signal available.",
      ...
      },
      {
        ...
        seeIt: "Click any citation chip and the source opens with the exact cited span highlighted. Grey markers in an answer were invented by the model and stripped."
      }
    ]
  },
  {
    ...
    topics: [
      {
        ...
        body: [
          ...
          "It also enables re-indexing without the original files: change the chunk size and the corpus is re-cut from stored text in seconds. The one thing re-indexing cannot do is re-parse, because the original bytes are gone, so changing the parser backend asks for a re-upload instead of pretending.",
          "Parsing is also where silent quality loss happens. A scanned PDF with no text layer extracts as nothing. It is detected and refused with a pointer to OCR, rather than indexed as an empty document that can never be retrieved."
        ],
      ...
      },
      {
        id: "chunking",
        title: "Chunk size",
        summary: "The unit of retrieval. It turns out to be mostly a generation concern.",
        body: [
    ...
    topics: [
      {
        ...
        body: [
          "All retrieval metrics score the same object: the ranked list of chunks, against labels saying where the answer actually lives. recall@k asks whether the labelled answer appears anywhere in the top k. That is the floor, since the generator cannot use what retrieval never surfaced. MRR (mean reciprocal rank) asks how high the first relevant chunk sat: 1 for first place, ½ for second, decaying fast, a proxy for “did we lead with the right thing”. nDCG credits every relevant chunk by position on a gentler curve.",
    ...
    topics: [
      {
        ...
        body: [
          "**Atomic table chunks.** The Phase 4 ablation measured structural chunking *hurting* financial-table questions: recall@5 fell from 0.545 to 0.182, because heading-bounded packing folds a statement's table into one large section chunk whose BM25 term statistics and embedding are diluted by surrounding prose. The fix falls out of the block model: in the structural chunker, a block of kind `table` becomes its own chunk instead of packing into its section, because a table's rows are a retrieval unit in a way prose paragraphs are not. The sec suite's `table`-tagged questions are the judge, already waiting.",
      ...
      },
      {
        ...
        summary: "Four experiments on the index itself: granularity, embedders (measured), compression, tokenisation.",
        body: [
          ...
          `**A second embedder**, measured. \`clear-rag ablate\` adds a dense-only and a hybrid + RRF row for each extra embedding model Ollama has pulled, on an index of its own: the embedder is the identity of an index, not a knob, so it rides on the row's provenance rather than its config hash. With vector search alone, recall@1 is ${m("retrieval", "dense only", 1, "recall")} for nomic-embed-text (768 dimensions), ${m("retrieval", "dense only, all-minilm", 1, "recall")} for all-minilm (384, the predecessor's family, a sixth of the size) and ${m("retrieval", "dense only, mxbai-embed-large", 1, "recall")} for mxbai-embed-large (1024). With keyword search fused in the three land at ${m("retrieval", "hybrid + RRF", 1, "recall")}, ${m("retrieval", "hybrid + RRF, all-minilm", 1, "recall")} and ${m("retrieval", "hybrid + RRF, mxbai-embed-large", 1, "recall")}. Embedding choice is the single most cargo-culted decision in RAG; on this corpus the smallest model is not the worst one, and fusion flattens the gap. Pin a row in Results to read it question by question.`,
