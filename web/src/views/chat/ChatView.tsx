...
export function ChatView() {
  ...
  useEffect(() => {
    // Opening a conversation with history lands at the bottom instantly, because a
            ...
            <article key={i} className="reveal mb-10">
              ...
              {turn.trace?.resolved_query && turn.trace.resolved_query !== turn.question ? (
                <p
                  ...
                  className="hint mt-2 font-mono text-meta text-subtle"
                >
              ...
              {expansionsOf(turn).length > 0 ? (
                <p
                  ...
                  className="hint mt-1 font-mono text-meta text-subtle"
                >
 ...
 * default, since watching retrieval is the app's point, but a long conversation can
...
function RankFlowSection({
  ...
  return (
    <div className="border-t border-line">
      ...
      >
        <span className="flex items-center gap-1.5 font-display text-meta tracking-[0.16em] text-subtle uppercase">
          <ChartLineIcon size={14} />
          Rank flow
        </span>
        <span className="flex items-center gap-2">
          {!open ? (
            <span className="text-meta text-subtle">how ranks changed between stages</span>
          ) : null}
          <CaretDownIcon
            size={14}
      ...
      {open ? (
        <>
          <p className="px-4 pb-1 text-meta text-subtle">
...
/** How many distinct chunks retrieval considered, which decides whether a chart earns space. */
